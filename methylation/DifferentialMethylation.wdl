...
# Given a PED file and a set of methylation bed files from pb-cpg-tools (PB) or modkit (ONT), 
...
workflow DifferentialMethylation {
    ...
    scatter (contig in contigs) {
        ...
        call DMR {
            input:
                pedFile = pedFile,
                smoothed_rds = Smooth.smoothed_rds,
        }
    }
...
task Smooth {
    ...
    RuntimeAttr runtime_default = object {
        ...
        max_retries: 1,
        boot_disk_gb: 10,
        docker: "quay.io/ymostovoy/lr-differential-methylation:1.2"
    }
    ...
    command <<<
        ...
        Rscript /smooth.R -d methylation/ -t ~{num_cores} -o $current_dir -p ~{pedFile}
...
task DMR {
    ...
    RuntimeAttr runtime_default = object {
        mem_gb: 32,
        disk_gb: ceil(size(smoothed_rds, "GB") + size(pedFile, "GB"))*5 + 20,
        cpu_cores: 4,
        preemptible_tries: 1,
        max_retries: 1,
        boot_disk_gb: 10,
    ...
    String contig = basename(smoothed_rds, "_smoothed.rds")
    ...
    command <<<
        ...
        Rscript /callDMRs.R -s ~{smoothed_rds} -p ~{pedFile} -t ~{num_cores} -o $current_dir
